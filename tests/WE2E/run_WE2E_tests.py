...
def run_we2e_tests(homedir, args) -> None:
    ...
    for test in tests_to_run:
        ...
        test_config_updates = {
            ...
            "workflow": {"COMPILER": args.compiler, "EXPT_SUBDIR": test_name},
        }
        ...
        if args.compiler == "gnu":
            ...
            if "task_run_post" in test_cfg:
                ...
                logging.info(
                    f"NUMX has been reset to 1 due to issues encountered with GNU compilers"
                )
            if "task_run_fcst" in test_cfg:
                test_cfg["task_run_fcst"].update({"ITASKS": 1})
                logging.info(
                    f"ITASKS has been reset to 1 due to issues encountered with GNU compilers"
                )
        ...
        expt_dir = generate_FV3LAM_wflow(
            ushdir, logfile=f"{ushdir}/log.generate_FV3LAM_wflow", debug=args.debug
        )
        if args.quiet:
            if args.debug:
...
def check_tests(tests: list) -> list:
    ...
    for testfile in testfiles:
        if os.path.basename(testfile) in testfilenames:
            duplicates = glob.glob(
                "test_configs/**/" + os.path.basename(testfile), recursive=True
            )
            raise Exception(
                dedent(
                    f"""
...
def check_task_get_extrn_bcs(
    ...
    if (
        ...
        if f"TEST_ALT_EXTRN_MDL_SYSBASEDIR_{I_OR_L}" in mach["platform"]:
            ...
            cfg_bcs_vars[f"EXTRN_MDL_SYSBASEDIR_{I_OR_L}"] = mach["platform"][
                f"TEST_ALT_EXTRN_MDL_SYSBASEDIR_{I_OR_L}"
            ]
        else:
            raise KeyError(
                "Non-default input file location "
    ...
    if cfg_bcs_vars[f"EXTRN_MDL_NAME_{I_OR_L}"] == "FV3GFS":
        if f"FV3GFS_FILE_FMT_{I_OR_L}" not in cfg_bcs_vars:
            cfg_bcs_vars[f"FV3GFS_FILE_FMT_{I_OR_L}"] = dflt[
                f"task_get_extrn_{ics_or_lbcs}"
            ]["envvars"][f"FV3GFS_FILE_FMT_{I_OR_L}"]
        cfg_bcs_vars[f"EXTRN_MDL_SOURCE_BASEDIR_{I_OR_L}"] = os.path.join(
            f"{mach['platform']['TEST_EXTRN_MDL_SOURCE_BASEDIR']}",
            f"{cfg_bcs_vars[f'EXTRN_MDL_NAME_{I_OR_L}']}",
            f"{cfg_bcs_vars[f'FV3GFS_FILE_FMT_{I_OR_L}']}",
            f"${{yyyymmddhh}}",
        )
    else:
        cfg_bcs_vars[f"EXTRN_MDL_SOURCE_BASEDIR_{I_OR_L}"] = os.path.join(
